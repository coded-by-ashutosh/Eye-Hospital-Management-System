...
const user =
{
    ...
    Fetch_User_By_Id: (data, callback) => {
        const q = `select * from user where id='${data.Id}'`

        db.query(q, callback)
    },
    Recent_Users: (callback) => {
        const q = `select * from user order by id desc limit 5`
        db.query(q, callback)
    },
}
